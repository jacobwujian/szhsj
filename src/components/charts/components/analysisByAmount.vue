  ...
  methods: {
    initChart() {
      ...
      myChart.on('click', params => {
        console.log(params)
        if (params.name == '小于1') {
          this.$emit('queryData', { type: 6, mixMoney: '', maxMoney: 1000000 })
        } else if (params.name == '1~5') {
          this.$emit('queryData', { type: 6, mixMoney: 1000000, maxMoney: 5000000 })
        } else if (params.name == '5~10') {
          this.$emit('queryData', { type: 6, mixMoney: 5000000, maxMoney: 10000000 })
        } else if (params.name == '10~20') {
          this.$emit('queryData', { type: 6, mixMoney: 10000000, maxMoney: 20000000 })
        } else if (params.name == '大于等于20') {
          this.$emit('queryData', { type: 6, mixMoney: 20000000, maxMoney: '' })
        }
      })
    }
  }
