...
<template>
  <div class="base-page">
    <a-form-model v-if="canSearch" ref="searchForm" :model="form" :rules="rules" v-bind="formItemLayout">
      <a-row :gutter="[8, 8]">
        <a-col v-for="(item, index) in searchType" :key="index" :xl="6" :xxl="colSpan">
          <a-form-model-item :label="item.label" :prop="item.params" :label-col="item.labelCol ? item.labelCol : formItemLayout.labelCol" :wrapper-col="item.wrapperCol ? item.wrapperCol : formItemLayout.wrapperCol">
      ...
      <a-row class="btns-row">
        ...
        <a-col :span="6" class="text-right">
          <a-space>
            <a-button v-if="canOpen" type="primary" html-type="submit" @click="openModel()">
              核查说明
            </a-button>
            <a-button v-if="canExport" type="primary" html-type="submit" @click="handleDownload(columns, 'data', excelName)">
              导出
            </a-button>
    ...
    <a-table class="c-table" :columns="columnsFormat" :data-source="data" :pagination="pagination" :loading="loading" :scroll="scroll" :bordered="true" size="middle" :row-key="record => record.id" @change="handleTableChange" :row-class-name="(record, index) => (index % 2 === 1 ? 'Discoloration' : 'Discoloration2')">
