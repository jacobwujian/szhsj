...
<template>
	<div class="base-page">
		<a-form-model v-if="canSearch" ref="searchForm" :model="form" :rules="rules" v-bind="formItemLayout" style="background: black;display: flex;margin-bottom: 20px;">
			<a-row :gutter="[8, 8]" style="width: 1200px;">
				<a-col v-for="(item, index) in searchType" :key="index" :xl="8" :xxl="colSpan" style="width: 400px;height: 45px;">
					<a-form-model-item
						style="height: 45px;"
						:label="item.label"
						:prop="item.params"
					...
					>
						...
						<a-select v-if="item.type === 'select'" v-model="form[item.params]" allow-clear :placeholder="`请选择${item.label}`">
							<a-select-option v-for="(op, i) in item.data" :key="`${i}-${op.value}`" :value="op.value">{{ op.name }}</a-select-option>
						</a-select>
						<a-range-picker v-if="item.type === 'datePicker'" v-model="form[item.params]" />
						<a-month-picker v-if="item.type === 'dateMonth'" v-model="form[item.params]" allow-clear />
						<a-cascader v-if="item.type === 'area'" v-model="form[item.params]" :options="areaOptions" :placeholder="item.placeholder" />
					</a-form-model-item>
				</a-col>
			</a-row>

			<a-row class="btns-row">
				<a-col :span="10" class="text-right" style="margin-top: 21px;padding-right: 15px;height: 45px;">
					<a-space>
						<a-button v-if="canOpen" type="primary" html-type="submit" @click="openModel()">核查说明</a-button>
						<a-button v-if="canExport" type="primary" html-type="submit" @click="handleDownload(columns, 'data', excelName)">导出</a-button>
						<a-col :span="5"><slot name="headerLeft" /></a-col>
						<slot name="headerLeft2" />
						<a-button type="primary" html-type="submit" :disabled="loading" @click="submitForm">查询</a-button>
		...
		>
			<template v-for="(colCustom, key) in columnsCustom" :slot="colCustom.customRender" slot-scope="text, record, index">
				<template v-if="colCustom.type === 'select'">
					<a-tag v-if="colCustom.customRender === 'chainStatus'" :key="key" :color="getColor(text)">{{ EnumMap(colCustom.data, text === null ? 'notKey' : text) }}</a-tag>
				...
				<span v-else-if="colCustom.type === 'time'" :key="key" :title="text && $dayjs(text).format(colCustom.format || 'YYYY-MM-DD HH:mm:ss')">
					{{ 'sas' + text && $dayjs(text).format(colCustom.format || 'YYYY-MM-DD HH:mm:ss') }}
				</span>
				<template v-else-if="colCustom.type === 'price'">
					{{ text == 0 ? '/' : text && text / 100 }}
				</template>
				<span v-else-if="colCustom.type === 'area'" :key="key" :title="codeToText(text)">{{ codeToText(text) }}</span>
	...
	data() {
		return {
			...
			pagination: {
				...
				size: 'middle',
				itemRender: (current, type, ele) => {
					console.log(type)
					if(type == 'jump-next'){
						return <a class='ant-pagination-item-link'><div class="ant-pagination-item-container"><i aria-label="图标: double-right" class="anticon anticon-double-right ant-pagination-item-link-icon" style="width:32px;color:white;height:32px;line-height:32px;color:bule;margin-top:8px">》</i><span class="ant-pagination-item-ellipsis">•••</span></div></a>
					}
					if(type == 'jump-prev'){
						return <a class='ant-pagination-item-link'><div class="ant-pagination-item-container"><i aria-label="图标: double-left" class="anticon anticon-double-left ant-pagination-item-link-icon" style="width:32px;color:white;height:32px;line-height:32px;color:bule;margin-top:8px">《</i><span class="ant-pagination-item-ellipsis">•••</span></div></a>
					}
					return ele
				}
			},
	...
	methods: {
		...
		initForm() {
			this.searchType.forEach(item => {
				this.$set(this.form, item.params, item.default || (item.type === 'datePicker' ? [] : undefined))
			})
		},
		submitForm() {
