<template>
  <basePage :loading="loading" :empty-state="emptyState">
      ...
      <a-form-model layout="inline" class="tempForm" @submit="install" @submit.native.prevent>
        ...
        <a-form-model-item label="状态">
          <a-select v-model="chooseType" class="inputCommon" style="width: 150px;font-size: 16px;">
            <template #suffixIcon />
            <a-select-option v-for="item in contractStatus" :key="item.value" :value="item.value">
              {{ item.name }}
            </a-select-option>
          </a-select>
        </a-form-model-item>
    ...
    <template slot="chart">
      <AnalyTitle title-name="授信额度异动分析" :refresh-status="false" @refresh="queryData" />
  ...
  data() {
    return {
      ...
      data: [
        ...
        { name: '异动涉及金额（亿元）', data: [], color: 'rgb(49, 249, 250)', areaCor: 'rgba(49, 249, 250,1)' }
      ],
      chooseType: '90',
      nowNumber: 0,
      lastNumber: 0,
  ...
  methods: {
    ...
    install() {
      this.sendData = {
        ...
        endTime: this.end,
        contractStates: this.chooseType
      }
