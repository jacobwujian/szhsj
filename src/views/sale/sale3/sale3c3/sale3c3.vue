  ...
  <basePage :loading="loading" :empty-state="emptyState">
    <template slot="header">
      ...
      <a-form-model layout="inline" class="tempForm" @submit="install" @submit.native.prevent>
        ...
        <a-form-model-item label="短溢装率阈值（%）">
    ...
    <template slot="chart">
      <AnalyTitle title-name="销售合同逾期异动分析" :refresh-status="false" @refresh="queryData" />
...
import { businessType, executStatus, creditTradeType, businessMode, issinglecredit } from '@/utils/typeEnum.js'
...
const columns = [
  ...
  },
  {
    ...
    width: 80,
    scopedSlots: { customRender: 'tradeTypeCode', type: 'select', data: creditTradeType }
  },
  {
    title: '业务类型',
  ...
  {
    title: '授信方式',
    dataIndex: 'creditModeCode',
    width: 200,
    scopedSlots: { customRender: 'creditModeCode', type: 'select', data: issinglecredit }
  },
  {
    title: '部门',
    dataIndex: 'deptName',
  ...
  {
    title: '数据执行率',
    dataIndex: 'dataExecutionRate',
    width: 100
  },
  {
    title: '短溢装率',
    dataIndex: 'numbOverRate',
    width: 100
  },
  // {
  //   title: '保证金比例',
  ...
  {
    title: '保证金收款期限',
    dataIndex: 'bailPaymentDate',
    width: 150,
    scopedSlots: { customRender: 'bailPaymentDate', type: 'time', format: 'YYYY-MM-DD' }
  },
  {
    title: '总数量',
    dataIndex: 'totalNum',
...
export default {
  ...
  data() {
    return {
      ...
      axis: [],
      data: [
        { name: '异动销售合同逾期数', data: [], color: 'rgb(71, 79, 223)', areaCor: 'rgba(71, 79, 223,1)' },
